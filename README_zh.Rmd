...
```
当 Conda 环境构建完成后，请下载我们提供的位于 `inst/` 文件夹下的参考基因组文件，或者建议自行构建 Bismark/STAR 所需的参考基因组文件。这些文件路径需要在 `BeaverGandalf$new()` 中指定，相关参数包括 `genomeFile`、`gnome_fasta`、`cgGR_gz`、`CGI`、`rnaseq_gtf` 和 `rnaseq_ref`。
